...
export const SoundshedPresety: React.FC = () => {
  ...
  if (duvod) {
    return (
      <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4">
        ...
        <p className="text-drobne text-neutral-400">{duvod}</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4 space-y-3">
      ...
      {!nalezene.length && (
        <p className="text-drobne text-neutral-500">Nic takového tu není.</p>
      )}

      <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
        {skupiny.map(([kategorie, vSkupine]) => (
          <div key={kategorie} className="space-y-1">
            <p className="odznak text-neutral-500 sticky top-0 bg-podklad py-1">
                    ...
                    <div className="px-3 pb-3 pt-1 space-y-2 border-t border-white/[0.06]">
                      {p.znacky.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {p.znacky.map((z) => (
                            <span key={z} className="text-stitek text-neutral-400 bg-white/5 border border-white/10 rounded-md px-1.5 py-0.5">
                              {z}
                            </span>
                          ))}
                        </div>
                      ...
                      {p.sceny.map((s) => (
                        <div key={s.id} className="space-y-1">
                          <p className="text-stitek text-neutral-400">{s.nazev}</p>
                          <div className="flex flex-wrap items-center gap-1">
                            {s.retezec.length === 0 && (
                              <span className="text-stitek text-neutral-600">prázdný řetězec</span>
                            )}
                            {s.retezec.map((c, i) => (
