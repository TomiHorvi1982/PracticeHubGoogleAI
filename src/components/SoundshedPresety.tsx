...
const barvaClanku = (k: string) => BARVA[k] || 'text-pismo-tlum border-white/10 bg-white/5';
  ...
  return (
    <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-4 space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-stitek text-pismo-slaby shrink-0">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-black/30 border border-white/[0.08] rounded-xl pl-9 pr-3 py-2 text-drobne text-white placeholder:text-pismo-slaby focus:outline-none focus:border-white/25"
        />
      </div>

      {!nalezene.length && (
        <p className="text-drobne text-pismo-slaby">Nic takového tu není.</p>
      )}

      <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
        {skupiny.map(([kategorie, vSkupine]) => (
          <div key={kategorie} className="space-y-1">
            <p className="odznak text-pismo-slaby sticky top-0 bg-podklad py-1">
            ...
            {vSkupine.map((p) => {
              ...
              return (
                <div key={p.id} className="rounded-xl border border-white/[0.08] bg-black/20 overflow-hidden">
                  ...
                  >
                    <ChevronRight
                      className={`w-3 h-3 text-pismo-slaby shrink-0 transition-transform ${otevreny ? 'rotate-90' : ''}`}
                    />
                    <span className="text-drobne text-pismo truncate flex-1">{p.nazev}</span>
                    ...
                    <div className="px-3 pb-3 pt-1 space-y-2 border-t border-white/[0.06]">
                      {p.znacky.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {p.znacky.map((z) => (
                            <span key={z} className="text-stitek text-pismo-tlum bg-white/5 border border-white/10 rounded-md px-1.5 py-0.5">
                              {z}
                            </span>
                          ))}
                        </div>
                      ...
                      {p.sceny.map((s) => (
                        <div key={s.id} className="space-y-1">
                          <p className="text-stitek text-pismo-tlum">{s.nazev}</p>
                          <div className="flex flex-wrap items-center gap-1">
                            {s.retezec.length === 0 && (
                              <span className="text-stitek text-pismo-slaby">prázdný řetězec</span>
                            )}
                            {s.retezec.map((c, i) => (
                              <React.Fragment key={`${s.id}-${i}`}>
                                {i > 0 && <span className="text-neutral-700 text-stitek">→</span>}
                                <span className={`text-stitek border rounded-md px-1.5 py-0.5 ${barvaClanku(c.kategorie)}`}>
                                  {c.nazev}
                                </span>
                              </React.Fragment>
                            ))}
