...
export const LibrarySection: React.FC<LibrarySectionProps> = ({
  ...
  // Stored Library items
  const [libraryItems, setLibraryItems] = useState<LibraryItem[]>(() => {
    if (typeof localStorage !== 'undefined') {
      const saved = localStorage.getItem('strumos_multimedia_library');
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          if (Array.isArray(parsed) && parsed.length > 0) return parsed;
        } catch (e) {
          console.error('Failed to parse multimedia library', e);
        }
      }
    }
  ...
  useEffect(() => {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('strumos_multimedia_library', JSON.stringify(libraryItems));
    }
  }, [libraryItems]);
  ...
  const handleDeleteItem = (itemId: string) => {
    ...
    if (activeItem?.id === itemId) {
      const remaining = libraryItems.filter((i) => i.id !== itemId);
