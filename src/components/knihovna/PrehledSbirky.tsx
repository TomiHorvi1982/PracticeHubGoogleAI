        ...
        <div className="max-h-[40vh] overflow-y-auto divide-y divide-white/[0.04]">
          ...
          {interpreti.map((i) => (
            ...
            >
              ...
              <span className="text-[10px] font-mono tabular-nums text-neutral-500">{i.count}</span>
      ...
      {!nacitam && vybrany && (
        <div className="space-y-2">
          <button
            ...
            className="text-[11px] text-[#FF9F0A] hover:underline cursor-pointer"
          >
            ← zpět na interprety
          </button>
            ...
            {skladby.map((s) => (
              <div key={s.id} className="flex items-center gap-2 py-2 group">
                <span className={`flex-1 truncate text-xs ${s.stored ? 'text-neutral-200' : 'text-neutral-500'}`}>
                  ...
                  {!s.stored && <span className="ml-2 text-[10px] text-neutral-600">jen v rejstříku</span>}
                </span>
                <span className="text-[10px] font-mono uppercase text-neutral-600">{s.format}</span>
                {s.stored && onOtevrit && (
                  <button
                    onClick={() => onOtevrit(s)}
