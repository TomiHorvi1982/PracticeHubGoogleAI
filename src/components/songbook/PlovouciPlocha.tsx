            ...
            <div className="absolute left-0 top-full mt-1 z-[100] bg-plocha-2 border border-white/[0.12] rounded-2xl shadow-2xl p-1.5 min-w-[200px]">
              {(Object.keys(POPIS_OKEN) as TypOkna[]).map((typ) => (
                <button
                  key={typ}
                  onClick={() => pridej(typ)}
                  className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-left text-drobne text-pismo hover:bg-white/10 hover:text-white cursor-pointer transition-all"
                >
        ...
        {okna.length > 0 && (
          <>
            <button
              ...
              className="px-3 py-2 bg-white/[0.06] hover:bg-white/[0.12] border border-white/[0.1] text-pismo text-drobne font-semibold rounded-xl flex items-center gap-1.5 cursor-pointer transition-all"
